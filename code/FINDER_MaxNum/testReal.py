...
def GetSolution(STEPRATIO, MODEL_FILE_CKPT):
    ...
    #model_file_path = '/content/FINDER/code/FINDER_MaxNum/models/barabasi_albert/'
    #model_file_ckpt = MODEL_FILE_CKPT
    model_file = MODEL_FILE_CKPT
    ## save_dir
    save_dir = '../results/MaxNum/'
...
def EvaluateSolution(STEPRATIO, MODEL_FILE_CKPT, STRTEGYID):
    ...
    #     data_test_name = ['Crime', 'HI-II-14', 'Digg', 'Enron', 'Gnutella31', 'Epinions', 'Facebook', 'Youtube', 'Flickr']
    data_test_name = [f for f in listdir(data_test_path) if isfile(join(data_test_path, f))]
    save_dir = '../results/MaxNum/StepRatio_%.4f/'%STEPRATIO
    ## begin computing...
    ...
    for i in range(len(data_test_name)):
        ...
        score, MaxCCList = dqn.EvaluateSol(data_test, solution, strategyID, reInsertStep=0)
        t2 = time.time()
        df.iloc[0, i] = score
        df.iloc[1, i] = t2 - t1
        result_file = save_dir + 'MaxCCList/MaxCCList_Strategy_' + data_test_name[i]
...
def findModel():
    ...
    VCFile = './models/%s/ModelVC_%d_%d.csv'%(g_type, NUM_MIN, NUM_MAX)
    vc_list = []
    ...
    best_model = './models/%s/nrange_%d_%d_iter_%d.ckpt' % (g_type, NUM_MIN, NUM_MAX, best_model_iter)
    return best_model
